...
int serveur::start()
{
	...
	if (protocol == TCP)
	{
		...
		while(1 && isStarted)
		{
			...
			pthread_create(&th, NULL, &serveur::beginconnected, (void*) myarg);
			pthread_detach(th);
		}
	}
	else
	{
		...
		while(1 && isStarted)
		{
			...
			pthread_create(&th, NULL, &serveur::beginreceived, (void*) myarg);
			pthread_detach(th);
		}
	}
	isStarted = 0;
...
int serveur::readline(int sock, unsigned char *buf)
{
...
int serveur::sread(int sock, unsigned char *buf, int limit)
{
	int size = 0;
	int tsize = 0;
	int nread = linesize;
...
// *** SET ***
void serveur::setProtocol(int proto)
{
	if (proto == TCP or proto == UDP)
		protocol = proto;
}

void serveur::setPort(int p) { port = p; }

void serveur::setParam(void *arg)
{
	param = arg;
}

void serveur::setEndStream(char *mastream)
{
	strncpy(EndStream, mastream, sizeof(EndStream)-1);
}

// *** GET ***
int serveur::getProtocol() { return protocol; }
int serveur::getPort() { return port; }
void serveur::getEndStream(char *mastream)
{
	mastream = EndStream;
}

// *** THREAD ***
void* serveur::beginconnected(void* data)
{
	struct arg_tcp *myarg;
	serveur *m_serv;
...
void* serveur::beginreceived(void* data)
{
	struct arg_udp *myarg;
	serveur *m_serv;
...
void serveur::received(int sock, char *msg, struct sockaddr_in info_udp) {}

void serveur::connected(int sock, struct sockaddr_in info_tcp) {}
